      ...
      <div class="page-body">
        <Form ref="queryForm" :label-width="80" label-position="left" inline>
          <Row>
            <Col span="16">
              <ButtonGroup>
                <Button @click="create" type="primary">{{L('Add')}}</Button>
                <Button @click="create" type="error">{{L('批量删除')}}</Button>
              </ButtonGroup>
            </Col>
            <Col span="8" class="searchmore">
            <Button @click="setsearchmoreDispaly" type="info" icon="ios-arrow-down" style="border-radius:0 4px 4px 0"></Button>
                <Input class="" v-model="pagerequest.keyword" @on-search="getpage" search enter-button placeholder="Enter something..." />
                
            </Col>
          </Row>
          <div v-show="searchmoreDispaly" style="margin-top:10px">
          <Row>
            <Col span="6">
          ...
          <Row style="text-align:right">
            <Button @click="search" type="primary">{{L('查询')}}</Button>
          </Row>
          </div>
        </Form>
...
export default class Users extends AbpBase {
  ...
  searchmoreDispaly:boolean=false;
    setsearchmoreDispaly(){
      this.searchmoreDispaly=!this.searchmoreDispaly;
    }
    search() { }
    get loading() {
