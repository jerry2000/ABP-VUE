...
class UserModule extends ListModule<UserState,any,User>{
    ...
    actions={
        ...
        async getUsersByOrgId(context:ActionContext<UserState,any>,payload:any){
            context.state.loading=true;
            let reponse=await Ajax.get('/api/services/app/User/GetUserByOrgId',{params:payload.data});
            context.state.loading=false;
            let page=reponse.data.result as PageResult<User>;
            context.state.totalCount=page.totalCount;
            context.state.list=page.items;
